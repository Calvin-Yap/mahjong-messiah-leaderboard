...
export function AddPlayerModal({ existingIcons, onClose, onCreated }: AddPlayerModalProps) {
  ...
  async function handleSubmit() {
    ...
    if (!icon) return setError(`Please ${tab === 'emoji' ? 'select or enter an emoji' : 'upload an image'}.`);
    if (tab === 'emoji' && existingIcons.includes(icon)) {
      return setError('That icon is already used by another player. Please choose a different one.');
    }
    ...
    <Modal title="➕ Add New Player" onClose={onClose}>
      ...
      <div className="mb-3 flex gap-1 rounded-lg bg-ink-50 p-1">
        ...
        <button
          className={clsx(
            'flex-1 rounded-md py-1.5 text-sm font-semibold',
            tab === 'image' ? 'bg-brand-500 text-white' : 'text-ink-500'
          )}
      ...
      {tab === 'emoji' ? (
        ...
        <div>
          ...
          {imageDataUrl && (
            ...
            <img src={imageDataUrl} alt="Preview" className="mt-3 h-16 w-16 rounded-full object-cover" />
          )}
        </div>
      )}

      {error && (
        <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-lose">⚠️ {error}</p>
      )}
